...
/* ── WORKERS ANIMATION BACKGROUND ── */
function WorkersBackground() {
  ...
  useEffect(() => {
    ...
    const CX = W / 2, CY = H / 2;
    const GEAR_R = Math.min(W, H) * 0.13;
    ...
    // Worker colors: blue, black, red, pink
    const COLORS = ["#1A6FD4","#1A1A2E","#C8102E","#E05A8A","#2ECC71","#F39C12"];

    // Draw a proper worker figure at (x,y) facing direction, scale s
    const drawWorker = (x, y, s, color, armAngle, legPhase, isSuper) => {
      ctx.save();
      ctx.translate(x, y);
      ctx.scale(s, s);
      ...
      // Shadow
      ctx.beginPath();
      ctx.ellipse(0, 28, 10 * s, 3, 0, 0, Math.PI * 2);
      ctx.fillStyle = "rgba(0,0,0,0.1)";
      ctx.fill();

      // Legs
      const lSwing = Math.sin(legPhase) * 12;
      ctx.strokeStyle = color;
      ctx.lineWidth = 3.5;
      ctx.lineCap = "round";
      // left leg
      ctx.beginPath(); ctx.moveTo(-2, 14); ctx.lineTo(-6 + lSwing, 28); ctx.stroke();
      // right leg
      ctx.beginPath(); ctx.moveTo(2, 14); ctx.lineTo(6 - lSwing, 28); ctx.stroke();

      // Body
      ctx.beginPath();
      ctx.moveTo(-8, 14); ctx.lineTo(8, 14); ctx.lineTo(6, -2); ctx.lineTo(-6, -2); ctx.closePath();
      ctx.fillStyle = color;
      ctx.fill();
      ...
      ctx.lineWidth = 3.5;
      ctx.lineCap = "round";
      const aSwing = Math.sin(legPhase + Math.PI) * 18;
      // left arm
      ctx.beginPath(); ctx.moveTo(-6, 2); ctx.lineTo(-14 + aSwing, 14 + armAngle); ctx.stroke();
      // right arm
      ctx.beginPath(); ctx.moveTo(6, 2); ctx.lineTo(14 - aSwing, 14 - armAngle); ctx.stroke();

      // Neck
      ctx.beginPath(); ctx.moveTo(0, -2); ctx.lineTo(0, -7);
      ctx.strokeStyle = color; ctx.lineWidth = 3; ctx.stroke();

      // Head
      ctx.beginPath(); ctx.arc(0, -14, isSuper ? 11 : 8, 0, Math.PI * 2);
      ...
      ctx.beginPath(); ctx.arc(-3, -15, isSuper ? 3 : 2.2, 0, Math.PI * 2); ctx.fill();
      ctx.beginPath(); ctx.arc(3, -15, isSuper ? 3 : 2.2, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = "#333";
      ctx.beginPath(); ctx.arc(-3, -15, isSuper ? 1.5 : 1, 0, Math.PI * 2); ctx.fill();
      ctx.beginPath(); ctx.arc(3, -15, isSuper ? 1.5 : 1, 0, Math.PI * 2); ctx.fill();
      ...
      // Tool (wrench) for workers near gear
      if (armAngle > 5) {
        ctx.save();
        ctx.translate(14, 14 + armAngle);
    ...
    // Draw gear at center
    const drawGear = (cx, cy, r, teeth, rot, alpha) => {
      ctx.save();
      ...
      for (let i = 0; i < teeth; i++) {
        ...
        const a2 = a1 + toothW / r;
        const a3 = a1 + (2 * Math.PI) / teeth - toothW / r;
        const a4 = a1 + (2 * Math.PI) / teeth;
        ...
        ctx.lineTo(Math.cos(a4) * (r + toothH), Math.sin(a4) * (r + toothH));
        ctx.lineTo(Math.cos(a4) * r, Math.sin(a4) * r);
      }
      ctx.closePath();
      const grd = ctx.createRadialGradient(0, 0, innerR * 0.3, 0, 0, r + toothH);
      grd.addColorStop(0, "rgba(200,16,46,0.25)");
      grd.addColorStop(0.6, "rgba(200,16,46,0.12)");
      grd.addColorStop(1, "rgba(200,16,46,0.04)");
      ctx.fillStyle = grd;
      ctx.fill();
      ...
      ctx.fillStyle = "rgba(200,16,46,0.08)"; ctx.fill();
      ctx.strokeStyle = "rgba(200,16,46,0.2)"; ctx.lineWidth = 1.5; ctx.stroke();

      // Hub
      ctx.beginPath(); ctx.arc(0, 0, r * 0.18, 0, Math.PI * 2);
      ctx.fillStyle = "rgba(200,16,46,0.4)"; ctx.fill();
      ...
      for (let i = 0; i < 6; i++) {
        ...
        ctx.strokeStyle = "rgba(200,16,46,0.25)"; ctx.lineWidth = 2; ctx.stroke();
      }

      ctx.restore();
    };

    // Define workers: vary sizes, colors, starting positions, speeds
    const WORKER_DEFS = [
      // [startAngle, orbitR, speed, scale, colorIdx, isSuper, phaseOffset]
      { a: 0,    r: GEAR_R * 1.6, sp: 0.003,  s: 1.4,  c: 0, sup: false, ph: 0 },
      { a: 1.2,  r: GEAR_R * 1.5, sp: -0.004, s: 0.9,  c: 1, sup: false, ph: 1 },
      { a: 2.4,  r: GEAR_R * 1.8, sp: 0.0025, s: 1.1,  c: 2, sup: false, ph: 2 },
      { a: 3.6,  r: GEAR_R * 1.4, sp: -0.005, s: 0.75, c: 3, sup: false, ph: 0.5 },
      { a: 4.8,  r: GEAR_R * 1.7, sp: 0.0035, s: 1.25, c: 4, sup: false, ph: 1.5 },
      { a: 0.6,  r: GEAR_R * 1.55,sp: -0.003, s: 0.85, c: 5, sup: false, ph: 2.5 },
      { a: 1.8,  r: GEAR_R * 1.9, sp: 0.002,  s: 1.6,  c: 0, sup: false, ph: 0.8 },
      { a: 3.0,  r: GEAR_R * 1.3, sp: -0.006, s: 0.65, c: 2, sup: false, ph: 1.8 },
      // Supervisor — orbits farther out, bigger
      { a: Math.PI, r: GEAR_R * 2.4, sp: 0.0015, s: 2.0, c: 1, sup: true, ph: 0 },
    ];

    // Draw path lines (roads workers walk on)
    const drawPaths = () => {
      WORKER_DEFS.forEach((w) => {
        ctx.beginPath();
        ctx.ellipse(CX, CY, w.r, w.r * 0.38, 0, 0, Math.PI * 2);
    ...
    const draw = () => {
      ...
      // Gear glow
      const glow = ctx.createRadialGradient(CX, CY, 0, CX, CY, GEAR_R * 2.2);
      glow.addColorStop(0, "rgba(200,16,46,0.08)");
      glow.addColorStop(1, "rgba(200,16,46,0)");
      ctx.beginPath(); ctx.arc(CX, CY, GEAR_R * 2.2, 0, Math.PI * 2);
      ctx.fillStyle = glow; ctx.fill();

      // Secondary smaller gear
      drawGear(CX + GEAR_R * 1.35, CY - GEAR_R * 0.6, GEAR_R * 0.45, 10, -gearRot * 1.8, 0.5);

      // Main gear
      drawGear(CX, CY, GEAR_R, 16, gearRot, 0.85);

      // Draw workers (sorted by y for depth)
      const workerPositions = WORKER_DEFS.map((w) => {
        w.a += w.sp;
        const wx = CX + Math.cos(w.a) * w.r;
        const wy = CY + Math.sin(w.a) * w.r * 0.38;
        const legPh = frame * 0.15 + w.ph;
        // arm angle increases when close to gear
        const distToGear = Math.hypot(wx - CX, wy - CY);
        const armA = distToGear < GEAR_R * 1.8 ? 18 : 4;
        return { wx, wy, s: w.s, color: COLORS[w.c], legPh, armA, sup: w.sup };
      });

      workerPositions.sort((a, b) => a.wy - b.wy);
      workerPositions.forEach(({ wx, wy, s, color, legPh, armA, sup }) => {
        drawWorker(wx, wy, s, color, armA, legPh, sup);
      });
...
export default function BPKLanding() {
  const [form,setForm] = useState({industry:"",biz:"",email:"",painType:"",pain:""});
  ...
  const handleSubmit = async () => {
    ...
    try {
      await saveToSupabase({ industry:form.industry, business_name:form.biz, email:form.email, pain_category:form.painType, pain_description:form.pain, status:"Pending" });
      setSubmitted(true);
          ...
          <div className="waitlist-inner">
            <div>
              ...
              <div className="email-notice">
                <Icon.mail/>
                <div className="email-notice-text"><strong>You will receive an email</strong> at your business address once our team has reviewed and validated your pain point — with your next steps and founding access details.</div>
              </div>
            </div>
            <div className="form-box">
              ...
              {submitted ? (
                <div className="success-box">
                  ...
                  <div className="success-email-note"><Icon.mail/><span><strong>Check your business email.</strong> Once your pain point is validated, you'll receive a confirmation email with your product roadmap and founding access details.</span></div>
                </div>
              ) : (
